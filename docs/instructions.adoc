...
+
----
> cf push Exchange_CADUSD --random-route
> cf push Exchange_BTCUSD --random-route
----
This will publish 2 order books. Because they each register with Service Discovery, we can dynamically determine which symbols are available for trading just by monitoring the state of the service!
